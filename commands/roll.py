...
class Roll(commands.Cog):
    ...
    @commands.command(name = 'Betroll', help = 'Roll a dice and bet on a lucky number! Usage: !betroll <bet amount> <lucky number> [range of dice, default 20]')
    async def betroll(self, ctx: commands.Context, bet: int, lucky_number: int, range: int = 20):
        ...
        if bet > current_balance:
            return await ctx.send(f"You do not have enough balance to place that bet. Your current balance is {current_balance}.")
        ...
        if result == lucky_number:
            ...
            if change_success:
                await ctx.send(f"JACKPOT! You rolled a {result}! and won {Payout} coins! Your new balance is {new_balance}.")
            else:
            ...
            change_success = set_balance(user_id, new_balance)
            if change_success:
                await ctx.send(f"You rolled a {result}. Sorry, you lost {bet} coins. Your new balance is {new_balance}.")
            else:
                await ctx.send("Error updating balance.")
